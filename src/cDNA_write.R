...
cDNA_write <- function(filename, output){
  #' @description  
  ...
  #'  
  #'  @usage 
  #'  writtenTable = cDNA_write(myArgs[1], myArgs[2])
  #'  write.csv(writtenTable, paste0("../results/",myArgs[2],".csv"))
  ...
  system(paste0("split -l 4500 ",filename,".txt"))
  system("mkdir ../results/split_mRNA/")
  system("mv x* ../results/split_mRNA/")
  
  split_files = system("ls ../results/split_mRNA/x*", intern = TRUE)
  ...
  writtenCDNAtest = write_cdna(merge.cdna, output)
  
  return(writtenCDNAtest)
}
